...
class BufferTest extends TestCase
{
    ...
    public function testCreateStringBuffer()
    {
        ...
        $this->assertEquals('abcdabcdabcdabcd', $buffer->toString());
        $this->assertEquals(8, $buffer->length());

        $buffer = new Buffer('bcdabcdabcdabcd', 'hex');
        $this->assertEquals('0bcdabcdabcdabcd', $buffer->toString());
        $this->assertEquals(8, $buffer->length());

        $buffer = new Buffer('我是測試');
        $this->assertEquals('e68891e698afe6b8ace8a9a6', $buffer->toString());
    ...
    public function testCreateArrayBuffer()
    {
        ...
        $buffer = new Buffer(['Hello World', 'abcdabcdabcdabcd']);
        $this->assertEquals('48656c6c6f20576f726c6461626364616263646162636461626364', $buffer->toString());
    }
    ...
    public function testCreateIntegerBuffer()
    {
        ...
        $this->assertEquals('00000000000000000000', $buffer->toString());
        $this->assertEquals(10, $buffer->length());

        $buffer = new Buffer(15);
        $this->assertEquals('000000000000000000000000000000', $buffer->toString());
        $this->assertEquals(15, $buffer->length());

        $buffer = new Buffer(20);
        $this->assertEquals('0000000000000000000000000000000000000000', $buffer->toString());
    ...
    public function testConcat()
    {
        ...
        $this->assertEquals('48656c6c6f20576f726c646162636461626364616263646162636454657374596f01', $buffer->toString());
        $this->assertEquals(34, $buffer->length());

        $bufferB = new Buffer(['A lo ha']);
        $buffer->concat($bufferB);
        $this->assertEquals('48656c6c6f20576f726c646162636461626364616263646162636454657374596f0141206c6f206861', $buffer->toString());
        $this->assertEquals(41, $buffer->length());

        $bufferC = new Buffer(['Goog']);
        $buffer->concat($bufferC, ['yo']);
        $this->assertEquals('48656c6c6f20576f726c646162636461626364616263646162636454657374596f0141206c6f206861476f6f67796f', $buffer->toString());
